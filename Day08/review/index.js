const cafeMenu = [
  ...
  },
  {
    ...
    isIce: true,
    calories: 210,
    ingredients: ["딸기", "우유", "얼음", "설탕"],
  },
];

//1. 우유가 들어간 메뉴의 이름만 배열로 추출하기
const quiz1 = cafeMenu.filter((x) => x.ingredients.some((v) => v == "우유"));
//2. 에스프레소가 들어가지 않는 차가운 메뉴의 이름 배열 만들기
const quiz2 = cafeMenu.filter(
  (x) => x.ingredients.every((v) => v != "에스프레소") && x.isIce
);
//3. 칼로리가 100 미만이면서 우유가 들어가지 않은 메뉴의 이름 배열로 만들기
const quiz3 = cafeMenu.filter(
  (x) => x.ingredients.every((v) => v != "우유") && x.calories < 100
);
//4. "설탕" 상분이 들어간 메뉴만 이름과 가격 객체 배열로 만들기
